#include "../include/hw10-program2.h"
...
void hw10Program2(
    ...
    guardNotNull(type1WordCountOutputFilePath, "type1WordCountOutputFilePath", "hw10Program2");
    guardNotNull(type2WordCountOutputFilePath, "type2WordCountOutputFilePath", "hw10Program2");
    guardNotNull(semaphoreName, "semaphoreName", "hw10Program2");

    sem_t * const semaphore = safeSemOpen(semaphoreName, "hw10Program1");
    FILE * const pipe1ReadFile = safeFdopen(pipe1ReadFileDescriptor, "r", "hw10Program1");
    FILE * const pipe2WriteFile = safeFdopen(pipe2WriteFileDescriptor, "w", "hw10Program1");

    unsigned int type1WordCount = 0;
    unsigned int type2WordCount = 0;

    safeSemWait(semaphore, "hw10Program2");
    while (true) {
        ...
        safeFprintf(pipe2WriteFile, "hw10Program2", "%s\n", pigLatinWord);
        free(pigLatinWord);

        free(currentWord);
    }
    safeSemPost(semaphore, "hw10Program2");

    safeFprintf(pipe2WriteFile, "hw10Program2", "\n");

    fclose(pipe1ReadFile);
    fclose(pipe2WriteFile);

    FILE * const type1WordCountOutputFile = safeFopen(type1WordCountOutputFilePath, "w", "hw10Program2");
    FILE * const type2WordCountOutputFile = safeFopen(type2WordCountOutputFilePath, "w", "hw10Program2");

    safeFprintf(type1WordCountOutputFile, "hw10Program2", "%u\n", type1WordCount);
    safeFprintf(type2WordCountOutputFile, "hw10Program2", "%u\n", type2WordCount);

#include "../include/hw10-program0.h"
...
static void createHw10Semaphore(char * *outSemaphoreName, sem_t * *outSemaphore);

/**
 * Run CSCI 451 HW10 program 0.
 */
void hw10Program0(
    ...
    guardNotNull(program1Path, "program1Path", "hw10Program0");
    guardNotNull(program2Path, "program2Path", "hw10Program0");
    guardNotNull(program3Path, "program3Path", "hw10Program0");

    char *semaphoreName;
    sem_t *semaphore;
    createHw10Semaphore(&semaphoreName, &semaphore);

    int pipe1ReadFileDescriptor;
    int pipe1WriteFileDescriptor;
    safeCreatePipe(&pipe1ReadFileDescriptor, &pipe1WriteFileDescriptor, "hw10Program0");

    int pipe2ReadFileDescriptor;
    int pipe2WriteFileDescriptor;
    safeCreatePipe(&pipe2ReadFileDescriptor, &pipe2WriteFileDescriptor, "hw10Program0");
    ...
    safeSemWait(semaphore, "hw10Program0");

    pid_t const program1ProcessId = safeFork("hw10Program0");
    ...
    pid_t const program2ProcessId = safeFork("hw10Program0");
    ...
    safeWaitpid(program1ProcessId, NULL, 0, "hw10Program0");
    safeWaitpid(program2ProcessId, NULL, 0, "hw10Program0");

    pid_t const program3ProcessId = safeFork("hw10Program0");
    ...
    safeWaitpid(program3ProcessId, NULL, 0, "hw10Program0");

    safeSemClose(semaphore, "hw10Program0");
    free(semaphoreName);
...
static void createHw10Semaphore(char * * const outSemaphoreName, sem_t * * const outSemaphore) {
    char *semaphoreName;
    sem_t *semaphore;
    while (true) {
        semaphoreName = formatString(
            "/csci-451-hw10-aidanmatheney-73a9fdb179f8496ebe3635e86e035743-semaphore-%d%d%d%d",
            randomInt(0, INT_MAX),
            randomInt(0, INT_MAX),
            randomInt(0, INT_MAX),
            randomInt(0, INT_MAX)
